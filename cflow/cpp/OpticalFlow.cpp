		...
		for(int hh=0;hh<nInnerFPIterations;hh++)
		{
			...
			if(nChannels==1)
				for(int i=0;i<nPixels;i++)
				{
					...
					temp *= temp; // temp^2 instead of temp
					switch(noiseModel)
					{
					case GMixture:
...
//--------------------------------------------------------------------------------------
void OpticalFlow::Coarse2FineFlow(DImage &vx, DImage &vy, DImage &warpI2,const DImage &Im1,
				  const DImage &Im2, double alpha, double ratio, int minWidth,
