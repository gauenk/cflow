 ...
 */
  ...
  template <typename T>
  Image<T>* numpy_to_image(T* ndarray, int H, int W, int C);

  template <typename T>
  void image_to_numpy(const Image<T>& image, T* ndarray_ptr);
  
template <typename T>
void image_to_numpy(const Image<T>& image, T* ndarray_ptr) {

  /* access raw image array */
  int H = image.height();
  int W = image.width();
  ...
  for (size_t idh = 0; idh < H; idh++) {
	...
	size_t img_index = idc + idw*C + idh*W*C;
	size_t nd_index =  idc + idw*C + idh*W*C;
	ndarray_ptr[nd_index] = image_ptr[img_index];
      }
    }
  }
...
template <typename T>
Image<T>* numpy_to_image(T* ndarray_ptr, int H, int W, int C) {

  /*  allocate the image */
  Image<T>* image = new Image<T>(W,H,C);
  ...
  for (size_t idh = 0; idh < H; idh++) {
	...
	size_t img_index = idc + idw*C + idh*W*C;
	size_t nd_index =  idc + idw*C + idh*W*C;
	image_ptr[img_index] = ndarray_ptr[nd_index];
      }
    }
  }
