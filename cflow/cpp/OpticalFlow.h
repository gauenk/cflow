...
public:
	...
	static void DissembleFlow(const DImage& flow,DImage& vx,DImage& vy)
	{
        ...
        // static void test_numpy_call(){
	//   cout << "test_numpy_call (noargs)" << endl;
	// }


        // static void test_numpy_call(NPY_FLOAT64* numpy){
	//   cout << "test_numpy_call (NPY_FLOAT64)" << endl;
	// }

        static void test_numpy_call(PyObject* numpy){
	  // PyObject* iter_ = PyArray_IterNew(numpy);
	  // PyArrayIterObject* iter = (PyArrayIterObject*)iter_;
	  PyArrayObject* ndarray = (PyArrayObject*) numpy;
	  // NpyIter* iter = NpyIter_New(ndarray, NPY_ITER_READONLY,
	  // 			      NPY_KEEPORDER, NPY_NO_CASTING,NULL);
	  int ndim = PyArray_NDIM(ndarray);
	  int dtype = PyArray_TYPE(ndarray);
	  npy_intp* shape = PyArray_SHAPE(ndarray);
	  cout << "test_numpy_call (pyobject)" << endl;
	  ...
	  for (int i = 0; i < ndim; i++){
        ...
        static void test_numpy_call(double* numpy){
	  cout << "test_numpy_call (double)" << endl;
	}

        static void ComputeOpticalFlow(double* numpyImg1,
				       double* numpyImg2,
				       double* numpyFlow,
				       int H, int W, int C){

	  DImage* Im1 = numpy_to_image<double>(numpyImg1, H, W, C);
	  DImage* Im2 = numpy_to_image<double>(numpyImg2, H, W, C);
	  DImage* Flow = numpy_to_image<double>(numpyFlow, H, W, 2);
	  // Im1->imwrite("./image1.png");
	  ComputeOpticalFlow(*Im1,*Im2,*Flow);
	  // Flow->imwrite("./flow1.png");
	  image_to_numpy<double>((Image<double>)*Flow,numpyFlow);
	}
